    ...
    <main>
      <div>
          ...
          {selectedMove ? (
            <div className={styles.description}>
              ...
              <p className={styles.moveDescription}>
                {moveData.description ? moveData.description.replace("$effect_chance", moveData.effectChance) : null}
              </p>
              <div className={styles.moveMisc}>
                <div className={styles.moveStats}>
                  {moveData.pp ? <span className={styles.movePP}>PP:{moveData.pp}</span> : null}
                  {moveData.accuracy ? <span className={styles.moveAccuracy}>ACC:{moveData.accuracy}</span> : null}
                  {moveData.power ? <span className={styles.movePower}>PWR:{moveData.power}</span> : null}
                </div>
                <div className={styles.moveInfo}>
                  <span className={styles.moveType}>Type: {moveData.type}</span>
                  <span className={styles.moveDamageClass}>{moveData.damageClass}</span>
                </div>
              </div>
            </div>
          ) : null}
        ...
        <div className={styles.itemsAndDescription}>
          ...
          {selectedItem ? (
            <div className={styles.description}>
              <h1 className={styles.itemName}>{formatString(selectedItem.name)}</h1>
              <p className={styles.itemDescription}>{selectedItem.entry ? selectedItem.entry[0].short_effect : null}</p>
            </div>
          ) : null}
        </div>
        <div>
        ...
        {selectedMoves.map((move) => {
          return (
            <p
              onClick={() => {
                setSelectedMoves(selectedMoves.filter((move1) => move1 !== move));
              }}
            >
              {formatString(move)}
            </p>
